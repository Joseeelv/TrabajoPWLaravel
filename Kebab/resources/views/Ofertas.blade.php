        ...
        @foreach ($offers as $offer)
            <li>
                <form method="POST" action="{{ route('ofertas.activar') }}">
                    ...
                    <input type="image" width="100px" src="{{ asset('assets/images/productos/' . $offer->product->img_src) }}" alt="">
                </form>
                <p>Oferta: {{ $offer->product->product_name }}</p>
                <p>Precio: {{ $offer->cost }} <img src="{{ asset('assets/images/logo/DKS.png') }}" alt='DKS Logo' width='20px'></p>
